 ...
 * 以對齊 V2 語意（訂閱單走 active、V2 的 8 個 _payuni_resp_* meta）。
 * 反查用的商店訂單編號一律讀送出時落地的 _payuni_mer_trade_no，不從 _payuni_order_suffix 推導
 * ——該 meta 自 3.5.18（issue #131）起是「送出即取號」，當下值已非送出時所用。
 *
 * @package payuni
 */
...
final class PendingReconciler {
	 ...
	 *
	 * 優先讀送出當下落地的 _payuni_mer_trade_no。本類別排程的訂單一定有這個 meta
	 * （Request 在送出時就寫入），所以正常流程永遠走這條。
	 *
	 * 下方的重建只是舊訂單的保底，且**只對 3.5.18 之前的訂單成立**：
	 * 那時 _payuni_order_suffix 是「成功才 +1」，pending 訂單的值就等於送出時用的值。
	 * 3.5.18 起改為「送出即取號」（Request::burn_order_suffix()），送出當下就 +1，
	 * 此時重建會得到比實際大 1 的編號。這類訂單（3.5.18 ~ 本功能上線之間建立、
	 * 仍 pending、且沒有 _payuni_mer_trade_no）不會被本類別排程，因此不影響正常流程；
	 * 若日後要做存量訂單的人工反查工具，不能沿用這個重建規則。
	 *
	 * @param \WC_Order $order 訂單物件。
	 *
	 * @return string
