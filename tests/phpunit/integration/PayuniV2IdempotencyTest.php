...
final class PayuniV2IdempotencyTest extends WP_UnitTestCase {
	...
	public function test_first_success_completes_order(): void {
		...
		// complete_paid_order() 不碰 _payuni_order_suffix：自 3.5.18（issue #131）起商店訂單編號
		// 改為「送出即取號」，遞增由 Request::burn_order_suffix() 在送出當下負責。
		// 在付款完成時再加一次會讓序號每筆成功交易跳兩號，日後反查對不上。
		$this->assertSame( '', (string) $reloaded->get_meta( '_payuni_order_suffix' ), 'complete_paid_order() 不應改動 suffix' );
		$this->assertSame(
			$order->get_id(),
	...
	/**
	 * @testdox 第二次收到相同成功通知應回傳 false，且交易紀錄備註只有一則
	 */
	public function test_second_success_is_skipped_and_does_not_increment_suffix(): void {
		...
		$reloaded = wc_get_order( $order->get_id() );
		$this->assertSame( '', (string) $reloaded->get_meta( '_payuni_order_suffix' ), 'suffix 不歸 complete_paid_order() 管，重複通知同樣不應改動它' );
	...
	/**
	 * @testdox notify 與 return 幾乎同時抵達時，訂單只完成一次、交易紀錄備註只有一則
	 */
	public function test_notify_then_return_completes_once(): void {
		...
		$this->assertSame( '', (string) $reloaded->get_meta( '_payuni_order_suffix' ), '兩個入口先後進來，都不應改動 suffix（遞增在送出當下已完成）' );
